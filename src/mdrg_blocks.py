...
def ablation_variants():
    """The nine variants reported in Table 7, as column-index arrays."""
...
# Table 2 of the paper. 60 of the 64 recorded channels are assigned.
REGION_MAP = {
    ...
    "L_Temporal":       ["FT7", "T7", "TP9"],
    "R_Temporal":       ["FT8", "T8", "TP10"],
...
# Not assigned to any region: each falls between two adjacent anatomical
# groupings, and assigning it to either would make that region inconsistent
# with its contralateral counterpart. These four channels feed the raw EEG
# branch, which uses all 64 channels, but not the MDRG branch.
UNASSIGNED_CHANNELS = ["FT9", "FT10", "TP7", "TP8"]
...
if __name__ == "__main__":
    ...
    print("Channels assigned:", sum(len(v) for v in REGION_MAP.values()),
          "of 64; unassigned:", UNASSIGNED_CHANNELS)
